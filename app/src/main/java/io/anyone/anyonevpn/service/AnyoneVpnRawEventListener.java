...
public class AnyoneVpnRawEventListener implements RawEventListener {
    private final AnyoneVpnService mService;
    ...
    AnyoneVpnRawEventListener(AnyoneVpnService service) {
        mService = service;
    ...
    public void onEvent(String keyword, String data) {
        ...
        if (AnonControlCommands.EVENT_BANDWIDTH_USED.equals(keyword)) {
            ...
            // don't bother looking up internal circuits that Anyone VPN clients won't directly use
    ...
    private void handleBandwidth(long read, long written) {
        String message = AnyoneVpnService.formatBandwidthCount(mService, read) + " ↓ / " + AnyoneVpnService.formatBandwidthCount(mService, written) + " ↑";
