package io.anyone.anyonevpn.utils
...
import io.anyone.anyonevpn.ui.hostedservices.HostedServicesContentProvider
import io.anyone.anyonevpn.ui.clientauth.ClientAuthContentProvider
...
class BackupUtils(private val mContext: Context) {
    ...
    private val basePath: File
        get() = File(mContext.filesDir.absolutePath, AnyoneVpnConstants.ANON_SERVICES_DIR)
    ...
    fun createAuthBackup(domain: String?, keyHash: String?, backupFile: Uri): String? {
        val fileText = AnyoneVpnService.buildV3ClientAuthFile(domain, keyHash)
    ...
    // todo this doesn't export data for anons that anyone hosts which have authentication (not supported yet...)
